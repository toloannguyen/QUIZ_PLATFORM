...
import { getMaterialsByLecture, uploadMaterial, deleteMaterial } from '../api/materialApi';
...
export default function CourseDetailPage() {
  ...
  async function handleCreateLecture(e) {
    ...
    try {
      ...
      setLectureError(error.response?.data?.message || 'Tạo bài giảng thất bại');
    } finally {
  ...
  async function handleUploadMaterial(lectureId) {
    ...
    try {
      ...
      setUploadForms((prev) => ({ ...prev, [lectureId]: { title: '', file: null } }));
      await loadLectures();
    } catch (error) {
      setUploadError(error.response?.data?.message || 'Upload tài liệu thất bại');
    } finally {
  ...
  async function handleDeleteMaterial(materialId, lectureId) {
    ...
    try {
      ...
      const nextMaterials = (materialMap[lectureId] || []).filter((material) => material.id !== materialId);
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-blue-600">Chi tiết khóa học</p>
              <h1 className="mt-2 text-3xl font-bold text-slate-800">{course.title}</h1>
            </div>
            <Link to="/courses" className="rounded bg-slate-800 px-4 py-2 text-sm font-medium text-white hover:bg-slate-900">
              Quay lại
            </Link>
          </div>
          ...
          <div className="rounded-xl bg-slate-50 p-4">
            ...
            <p className="mt-2 text-slate-700">{course.description || 'Không có mô tả'}</p>
          </div>

          <div className="mt-6 grid gap-4 md:grid-cols-3">
            <div className="rounded-xl border border-slate-200 p-4">
              <p className="text-sm text-slate-500">ID</p>
              <p className="mt-2 text-xl font-semibold text-slate-800">#{course.id}</p>
            </div>
            <div className="rounded-xl border border-slate-200 p-4">
        ...
        {isTeacher && (
          <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <h2 className="text-xl font-semibold text-slate-800">Thêm bài giảng</h2>
            {lectureError && <div className="mt-3 rounded bg-red-50 px-3 py-2 text-sm text-red-600">{lectureError}</div>}

            <form onSubmit={handleCreateLecture} className="mt-5 grid gap-4 md:grid-cols-2">
              <div className="md:col-span-2">
                <label className="mb-1 block text-sm font-medium text-slate-700">Tiêu đề bài giảng</label>
                <input
        ...
        <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          <div className="mb-5 flex items-center justify-between">
            <h2 className="text-xl font-semibold text-slate-800">Bài giảng và tài liệu</h2>
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">{lectures.length} bài giảng</span>
          </div>

          {lectures.length === 0 ? (
            <p className="text-sm text-slate-500">Khóa học này chưa có bài giảng nào.</p>
          ) : (
            <div className="space-y-5">
              {lectures.map((lecture) => {
                ...
                return (
                  <div key={lecture.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Bài giảng #{lecture.id}</p>
                        <h3 className="mt-2 text-lg font-semibold text-slate-800">{lecture.title}</h3>
                        {lecture.description && <p className="mt-1 text-sm text-slate-600">{lecture.description}</p>}
                      </div>
                    </div>

                    <div className="mt-4">
                      <h4 className="text-sm font-semibold uppercase tracking-[0.15em] text-slate-500">Tài liệu hỗ trợ</h4>

                      {materials.length === 0 ? (
                        <p className="mt-2 text-sm text-slate-500">Chưa có tài liệu nào cho bài giảng này.</p>
                      ) : (
                        <ul className="mt-3 space-y-2">
                          {materials.map((material) => {
                            const backendBaseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000';
                            const fileUrl = new URL(material.fileUrl, backendBaseUrl).toString();

                            return (
                              <li key={material.id} className="flex flex-col gap-2 rounded-lg border border-slate-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                  <a
                                    href={fileUrl}
                                    target="_blank"
                                  ...
                                  <p className="text-xs text-slate-500">
                                    {material.fileName} · {material.aiProcessed ? 'AI đã xử lý' : material.aiError ? 'AI lỗi' : 'Đang xử lý'}
                                  </p>
                                </div>

                                {isTeacher && (
                                  <button
                                    onClick={() => handleDeleteMaterial(material.id, lecture.id)}
                    ...
                    {isTeacher && (
                      <div className="mt-4 rounded-lg border border-dashed border-slate-300 bg-white p-3">
                        <p className="text-sm font-medium text-slate-700">Upload tài liệu PDF cho bài giảng</p>
                        {uploadError && <div className="mt-2 rounded bg-red-50 px-3 py-2 text-xs text-red-600">{uploadError}</div>}
                        ...
                        <div className="mt-3 grid gap-3 md:grid-cols-[1.2fr_1fr_auto]">
                          ...
                          >
                            {uploadingMaterialId === lecture.id ? 'Đang tải...' : 'Upload'}
                          </button>
                        </div>
                      </div>
                    )}
